...
object Application extends Controller {
  ...
  /**
	...
	def searchNotes(title: Option[String], searchByIndentation: Boolean, textInside: Option[String], ps: PrintStream) 
	  ...
	  @tailrec
	  def goFindNotes(offset: Int, startMonth: String)(implicit filter: NoteFilter): (String, Int) = {
  		...
  		val endNoteAggregation = aggregateByMonthAndPrintCounts(noteChunk, startMonth, searchByIndentation, textInside, ps)
  		val endMonth = endNoteAggregation._1
  		
  		val newOffset = offset + noteChunk.length
  		if (newOffset < totalNoteCount)
  		  goFindNotes(newOffset, endMonth)
  		else 
  		  endNoteAggregation
	  }
	  ...
	  val query = s"$titleFilter $textInsideFilter"
		...
		val endNoteAggregation = goFindNotes(0, startFakeMonth)
	...
	def aggregateByMonthAndPrintCounts(notes: java.util.List[Note], startMonth: String, searchByIndentation: Boolean, 
		...
		val endNoteAggregation = notes.foldLeft((startMonth, 0)) { case ((currentMonth, currentMonthCount), note) =>
	...
	def getMonth(note: Note) = {
	  val createdTs = note.getUpdated
	  val c = Calendar.getInstance
	  c.setTimeInMillis(createdTs)
	  (c.get(Calendar.MONTH) + 1) + "/" + c.get(Calendar.YEAR) 
	}
	
  def getFeatureCountByNote(note: Note, searchByIndentation: Boolean, textInside: Option[String])
    (implicit noteStore: NoteStore.Client, token: String): Int = {
    @tailrec
